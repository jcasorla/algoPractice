...
function processData(input) {
    ...
    for(let i = 1; i < tempArray.length; i++) {
        const [key, value] = tempArray[i].split(' ');
        if(value !== undefined) {
            map.set(key, +value);
        } else {
            const searchedString = tempArray[i];
            console.log(validator(searchedString, map));
        }
    }
} 
function validator(key, map) { if(map.has(key)) { const value = map.get(key); return key+'='+value } return 'Not found'; };
